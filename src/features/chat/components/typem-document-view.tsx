...
import { buildDocumentPdf } from '@/lib/typem-pdf';
...
export function TypeMDocumentView({
  ...
  const handleDownloadPdf = () => {
    buildDocumentPdf(
      ...
      (title.replace(/[^a-z0-9]/gi, '-') || 'document').slice(0, 50)
    );
  };
  ...
  if (sections.length === 0) {
    return (
      <div
        className={cn(
          'rounded-xl border border-amber-200/70 dark:border-[#2d2a26] overflow-hidden',
          'bg-[#fefdfb] dark:bg-[#1a1815] text-sm px-3 py-4 text-center text-stone-500 dark:text-stone-400',
          className
        )}
      >
        No sections yet.
  ...
  return (
    <div
      className={cn(
        ...
        'bg-[#fefdfb] dark:bg-[#1a1815] shadow-sm min-w-0 max-w-full',
        className
      )}
    >
      <div className="flex items-center justify-between gap-2 px-3 py-2.5 border-b border-amber-100 dark:border-[#2d2a26] bg-amber-50/60 dark:bg-[#171412]">
        <div className="flex items-center gap-2">
          <div className="flex items-center justify-center w-8 h-8 rounded-lg bg-amber-100 dark:bg-[#1f1c19] border border-amber-200/60 dark:border-[#2d2a26]">
      ...
      <div className="divide-y divide-amber-100 dark:divide-[#2d2a26]">
